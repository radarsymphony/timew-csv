...
REPORT_PATH=${REPORT_PATH:-$HOME/timew-reports/}
USER_NAME=${USER_NAME:-${USER}}
SEP=${SEP:-';'}

## Variables
total_seconds=0
report_file_name="${USER_NAME}-$(date -dlast-monday +%F)-weekly-log.csv"
tmp_json=$(mktemp)
tmp_csv=$(mktemp)

## Create weekly log file
mkdir -p "${REPORT_PATH}"
[[ -e "${REPORT_PATH}"/"${report_file_name}" ]] && rm "${REPORT_PATH}"/"$Documents/SelfDesign/weekly-logs{report_file_name}"
touch "${REPORT_PATH}"/"${report_file_name}"
...
reformat_csv() {
    ...
    echo "ID${SEP}Tasks/Tags${SEP}Remarks/Notes${SEP}Date${SEP}Start${SEP}End${SEP}Duration${SEP}Totals" > "${REPORT_PATH}"/"${report_file_name}"
    ...
    while IFS="," read -r id start end tags notes; do
        ...
        echo "${id}${SEP}${tags}${SEP}${notes}${SEP}${_date}${SEP}${hmn_start}${SEP}${hmn_end}${SEP}${duration}" >> "${REPORT_PATH}"/"${report_file_name}"

    done < <(tail -n +2 "${tmp_csv}")

    ## Add total hours to CSV on last line
    echo "${SEP}${SEP}${SEP}${SEP}${SEP}${SEP}${SEP}$(convert_secs ${total_seconds})${SEP}(HH:MM)" >> "${REPORT_PATH}"/"${report_file_name}"
}
...
echo -n "${REPORT_PATH}"/"${report_file_name}"
